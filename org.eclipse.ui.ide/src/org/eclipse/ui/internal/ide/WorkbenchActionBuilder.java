...
public final class WorkbenchActionBuilder extends ActionBarAdvisor {
	...
	private void hookListeners() {
		...
		// listen for project description changes, which can affect enablement of build
		// actions
	...
	private void makeFeatureDependentActions(IWorkbenchWindow window) {
		...
		// Optimization: if welcome pages were found on a previous run, then just add
		// the action.
		...
		// See if a tips and tricks page is specified.
		// Optimization: if tips and tricks were found on a previous run, then just add
		// the action.
	 ...
	 * @param infos
	 *            the infos
	 * @return <code>true</code> if a welcome page was found, <code>false</code> if
	 *         not
	 */
	 ...
	 *            <code>false</code> to queue the update to be run in the event loop
	 */
	void updateBuildActions(boolean immediately) {
		...
		if (immediately) {
			...
			// This helps to reduce flicker if autobuild is being temporarily disabled
			// programmatically.
