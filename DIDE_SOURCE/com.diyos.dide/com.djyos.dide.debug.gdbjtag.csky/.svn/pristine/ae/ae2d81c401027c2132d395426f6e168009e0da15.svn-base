...
//					DefaultPreferences.CSKY_IS_VERBOSE_DEFAULT)) {
//				lst.add("--verbose");
//			}
//
...
//	}
	public static String[] getGdbServerCommandLineArray(ILaunchConfiguration configuration) {
		...
		try {
			...
			// Added always, to get the 'Waiting for connection' message.			
			lst.add("-setclk");
			lst.add(Integer.toString(configuration.getAttribute(ConfigurationAttributes.GDB_SERVER_ICE_CLK,
							DefaultPreferences.SERVER_GDB_ICE_CLK_DEFAULT))+"k");

			lst.add("-mtcrdelay");
			lst.add(Integer.toString(configuration.getAttribute(ConfigurationAttributes.GDB_SERVER_DELAY_MTCR,
							DefaultPreferences.SERVER_GDB_DELAY_MTCR_DEFAULT)));
	        if (!configuration.getAttribute(ConfigurationAttributes.GDB_SERVER_USE_DDC, DefaultPreferences.SERVER_GDB_USE_DDC_DEFAULT))
	        {
	        	lst.add("-noddc");
	        }
...
//	public static String getCSkyBoardName(ILaunchConfiguration config) throws CoreException {
//
//		return config.getAttribute(ConfigurationAttributes.GDB_SERVER_BOARD_NAME, "").trim();
//	}
//
//	public static String getCSkyDeviceName(ILaunchConfiguration config) throws CoreException {
//
