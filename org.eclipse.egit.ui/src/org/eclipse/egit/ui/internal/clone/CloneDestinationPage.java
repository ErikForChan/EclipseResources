...
public class CloneDestinationPage extends WizardPage {
	...
	String didePath = fullPath.substring(6,
			(fullPath.substring(0, fullPath.length() - 1)).lastIndexOf("/") //$NON-NLS-1$
					+ 1);

	String djysrcPath = didePath + "djysrc"; //$NON-NLS-1$
	...
	private void revalidate(@NonNull RepositorySelection repoSelection,
		...
		if (!repoSelection.equals(validatedRepoSelection)) {
			...
			// final String n =
			// validatedRepoSelection.getURI().getHumanishName();
			// setDescription(
			// NLS.bind(UIText.CloneDestinationPage_description, n));
			...
			directoryText.setText(djysrcPath);
		}
